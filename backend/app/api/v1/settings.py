...
async def save_provider(
    ...
    if isinstance(safe_model, str) and safe_model.lower() in {m.lower() for m in deprecated_models}:
        safe_model = "llama-3.3-70b-versatile"  # ✅ fixed: was 'llama3-70b-8192' (also deprecated)
...
async def list_ollama_models():
    ...
    try:
        async with httpx.AsyncClient(timeout=5) as client:
            resp = await client.get(f"{cfg.OLLAMA_BASE_URL}/api/tags")  # ✅ fixed: was hardcoded localhost
            data = resp.json()
