...
async def save_provider(
    ...
    config = db.query(LLMConfigDB).filter(LLMConfigDB.user_id == user.id, LLMConfigDB.provider == req.provider).first()
...
async def test_provider(req: ProviderTestRequest, user: UserDB = Depends(require_user), db: Session = Depends(get_db)):
    ...
    if api_key == "********":
        config = (
            db.query(LLMConfigDB).filter(LLMConfigDB.user_id == user.id, LLMConfigDB.provider == req.provider).first()
        )
    ...
    try:
        response = await LLMProxy.complete(
            provider=req.provider, model=req.model, prompt="Say 'OK'", api_key=api_key, base_url=req.base_url
        )
...
async def delete_provider(
    ...
    config = db.query(LLMConfigDB).filter(LLMConfigDB.user_id == user.id, LLMConfigDB.provider == provider).first()
    if config:
        db.delete(config)
        db.commit()
    return {"success": True}
