...
class LLMProxy:
    ...
    async def complete(
        ...
        try:
            if provider == "ollama":
                ...
                return await LLMProxy._complete_openai_compatible(provider, model, prompt, system_prompt, api_key, temperature, max_tokens)
            elif provider == "anthropic":
                return await LLMProxy._complete_anthropic(model, prompt, system_prompt, api_key, temperature, max_tokens)
    ...
    async def stream(
        ...
        if provider == "ollama":
            ...
            async for chunk in LLMProxy._stream_openai_compatible(provider, model, prompt, system_prompt, api_key, temperature, max_tokens):
                yield chunk
        else:
            # Fallback to non-streaming for unsupported providers (or implement them)
            res = await LLMProxy.complete(provider, model, prompt, system_prompt, api_key, base_url, temperature, max_tokens)
            yield res

    @staticmethod
    async def _complete_ollama(model: str, prompt: str, system: str, base_url: Optional[str], temp: float, tokens: int) -> str:
    ...
    @staticmethod
    async def _stream_ollama(model: str, prompt: str, system: str, base_url: Optional[str], temp: float, tokens: int) -> AsyncGenerator[str, None]:
        url = (base_url or settings.OLLAMA_BASE_URL).rstrip("/") + "/api/generate"
        payload = {
            "model": model,
            "prompt": prompt,
    ...
    @staticmethod
    async def _complete_openai_compatible(provider: str, model: str, prompt: str, system: str, api_key: str, temp: float, tokens: int) -> str:
        bases = {
    ...
    @staticmethod
    async def _stream_openai_compatible(provider: str, model: str, prompt: str, system: str, api_key: str, temp: float, tokens: int) -> AsyncGenerator[str, None]:
        bases = {
            "openai": "https://api.openai.com/v1",
            "groq": "https://api.groq.com/openai/v1",
            "openrouter": "https://openrouter.ai/api/v1",
    ...
    async def _complete_anthropic(model: str, prompt: str, system: str, api_key: str, temp: float, tokens: int) -> str:
        ...
        payload = {"model": model, "messages": [{"role": "user", "content": prompt}], "max_tokens": tokens, "temperature": temp}
    ...
    async def _complete_gemini(model: str, prompt: str, system: str, api_key: str, temp: float, tokens: int) -> str:
        ...
        payload = {"contents": [{"parts": [{"text": full_prompt}]}], "generationConfig": {"temperature": temp, "maxOutputTokens": tokens}}
        async with httpx.AsyncClient(timeout=60) as client:
            resp = await client.post(url, json=payload)
            resp.raise_for_status()
