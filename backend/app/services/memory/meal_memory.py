...
def get_recent_meals(user_id: int, days: int = 3) -> List[Dict[str, Any]]:
    ...
    from app.models.user import SessionLocal, DailyLogDB
    ...
    try:
        ...
        meals = (
            db.query(DailyLogDB)
            .filter(DailyLogDB.user_id == user_id, DailyLogDB.log_date >= start_date)
            .order_by(DailyLogDB.log_date.desc(), DailyLogDB.created_at.desc())
            .all()
        )
...
def get_daily_summary(user_id: int, date: str = None) -> Dict[str, Any]:
    ...
    db = SessionLocal()
    try:
        meals = db.query(DailyLogDB).filter(DailyLogDB.user_id == user_id, DailyLogDB.log_date == date).all()
...
def get_weekly_trends(user_id: int) -> Dict[str, Any]:
    ...
    return {"days": len(by_date), "trends": by_date}
