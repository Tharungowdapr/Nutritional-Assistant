...
class LLMRouter:
    ...
    async def _check_ollama(self) -> bool:
        ...
        try:
            async with httpx.AsyncClient(timeout=3.0) as client:  # ✅ fixed: 1s was too aggressive
    ...
    async def generate(self, prompt: str, system: str = "",
        ...
        if self._active_provider == "ollama":
            try:
                return await self._generate_ollama(prompt, system, temperature, max_tokens), "ollama"
    ...
    async def stream_generate(self, prompt: str, system: str = "",
        ...
        if self._active_provider == "ollama":
            try:
                async for token in self._stream_ollama(prompt, system, temperature, max_tokens):
                    yield token
                return
            except Exception as e:
        ...
        if self._active_provider == "groq":
            try:
                async for token in self._stream_groq(prompt, system, temperature, max_tokens):
                    yield token
                return
            except Exception as e:
    ...
    async def _stream_ollama(self, prompt: str, system: str, temperature: float, max_tokens: int = 4096):
        payload = {
            "model": self.ollama_model,
            "prompt": prompt,
            "system": system,
    ...
    async def _generate_groq(self, prompt: str, system: str,
        ...
        payload = {
            ...
            "max_tokens": max_tokens,  # ✅ fixed: was hardcoded 2048
        }
